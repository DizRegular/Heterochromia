...
public class GameManipulator {
    ...
    public static int getQuadrant(Vector2D v) {
        ...
        // FIX: ตัวแปร Width และ Height เคยสลับกัน แก้ให้ถูกต้องแล้ว
        int quadrantWidth = GameManipulator.gameQuadrantWidth;
        int quadrantHeight = GameManipulator.gameQuadrantHeight;
        if ((v.getXCoord() < quadrantWidth / 2) && (v.getYCoord() < quadrantHeight / 2)) {
            return 0;
        } else if ((v.getXCoord() > quadrantWidth / 2) && (v.getYCoord() < quadrantHeight / 2)) {
            return 1;
        } else if ((v.getXCoord() < quadrantWidth / 2) && (v.getYCoord() > quadrantHeight / 2)) {
            return 2;
        } else {
    ...
    public static void sortPhysicObjectPosition(CollisionObject obj) {
         ...
         * And keep that information in quadrant container */
    ...
    private void setXCollsion(double XIntersect, KinematicObject k, CollisionObject otherObj, double XAxis) {
        ...
        if (XAxis >= 0) {
            // FIX: เคยใช้ getYCoord() แทน getXCoord() แก้ให้ถูกต้องแล้ว
